...
type TopSongsSongCardProps = {
  playlistSong: { song: Song; playlistId: string; songId: string };
  editMode: boolean;
  removeFromPlaylists: (songId: string) => Promise<void>;
};

const TopSongsSongCard = ({
  playlistSong,
  editMode,
  removeFromPlaylists,
}: TopSongsSongCardProps) => {
  return (
    <div className="flex items-center gap-2 w-full h-auto bg-boxLightGrey p-2 my-2">
      ...
      <div className="ml-auto">
        {editMode && (
          <button
            onClick={() => removeFromPlaylists(playlistSong.songId)}
...
export default TopSongsSongCard;
